...
BEGIN
    ...
    FOR r IN SELECT * FROM departments ORDER BY dept_no LOOP
        crc := MD5(CONCAT_WS('#',crc, r.dept_no,r.dept_name));
    END LOOP;
    ...
    FOR r IN SELECT * FROM dept_manager ORDER BY dept_no,emp_no LOOP
        crc := MD5(CONCAT_WS('#',crc, r.dept_no,r.emp_no, r.from_date,r.to_date));
    END LOOP;
    ...
    FOR r IN SELECT * FROM titles order by emp_no,title,from_date LOOP
        crc := MD5(CONCAT_WS('#',crc, r.emp_no, r.title, r.from_date,r.to_date));
    END LOOP;
    ...
    FOR r IN SELECT * FROM salaries order by emp_no,from_date,to_date LOOP
        crc := MD5(CONCAT_WS('#',crc, r.emp_no, r.salary, r.from_date,r.to_date));
    END LOOP;
...
select 'CRC' as summary,  CASE WHEN (select count(*) from expected_values e inner join found_values f on (e.table_name=f.table_name) where f.crc_md5 != e.crc_md5) = 0 THEN 'OK' ELSE 'FAIL' END as "result"
union all
select 'count', CASE WHEN (select count(*) from expected_values e inner join found_values f on (e.table_name=f.table_name) where f.recs != e.recs) = 0 THEN 'OK' ELSE 'FAIL' END;
